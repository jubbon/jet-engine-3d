...
const BYPASS_IN = [
  [X_START, 0.6], [-5.2, 0.62], [-3.95, 1.0], [-3.4, 1.08], [0.0, 1.09],
  [1.6, 0.99], [2.6, 0.92], [5.0, 0.88], [X_END, 0.85],
];
const BYPASS_OUT = [
  [X_START, 1.42], [-5.2, 1.55], [-3.95, 1.54], [-3.4, 1.66], [0.0, 1.7],
  [1.6, 1.5], [2.6, 1.48], [5.0, 1.6], [X_END, 1.85],
];
const CORE_IN = [
  [X_START, 0.06], [-5.2, 0.12], [-4.6, 0.35], [-3.95, 0.57], [-3.3, 0.59],
  [-2.6, 0.63], [-2.3, 0.52], [-0.55, 0.46], [-0.28, 0.47], [0.55, 0.51],
  [1.15, 0.47], [2.78, 0.6], [3.0, 0.57], [4.05, 0.42], [4.75, 0.06],
  [6.5, 0.02], [X_END, 0.02],
];
const CORE_OUT = [
  [X_START, 0.46], [-5.2, 0.55], [-4.6, 0.72], [-3.95, 0.94], [-3.3, 0.91],
  [-2.6, 0.87], [-2.3, 0.76], [-0.55, 0.55], [-0.28, 0.75], [0.55, 0.67],
  [1.15, 0.86], [2.78, 1.07], [3.0, 1.08], [4.05, 0.84], [4.75, 0.8],
  [6.5, 0.95], [X_END, 1.25],
];

// осевая скорость (усл. ед./с при 100 % режима)
const BYPASS_V = [
  [X_START, 2.4], [-5.2, 2.3], [-3.95, 2.5], [-3.0, 2.7], [1.0, 2.9],
  [1.6, 4.4], [3.0, 4.2], [X_END, 3.4],
];
const CORE_V = [
  [X_START, 2.4], [-5.2, 2.2], [-3.95, 2.0], [-2.3, 1.5], [-0.55, 0.95],
  [-0.28, 0.8], [0.55, 1.3], [1.15, 2.3], [2.78, 3.3], [4.05, 5.4],
  [6.0, 4.6], [X_END, 3.4],
];

// закрутка потока (рад/с)
const CORE_SWIRL = [
  [X_START, 0], [-4.0, 0], [-3.9, 2.6], [-2.4, 2.2], [-0.6, 1.6], [-0.3, 0.6],
  [0.6, 1.0], [0.8, 3.4], [2.8, 2.6], [3.1, 0.3], [X_END, 0.1],
];
const BYPASS_SWIRL = [
  [X_START, 0], [-4.0, 0], [-3.9, 2.4], [-3.3, 1.9], [-3.0, 0.35], [X_END, 0.2],
];

// относительная температура 0..1
const CORE_T = [
  [X_START, 0.02], [-3.95, 0.04], [-3.3, 0.09], [-2.6, 0.14], [-2.3, 0.16],
  [-0.55, 0.44], [-0.28, 0.46], [0.05, 0.96], [0.4, 1.0], [0.62, 0.92],
  [1.15, 0.78], [2.78, 0.6], [4.05, 0.54], [6.0, 0.4], [X_END, 0.26],
];
const BYPASS_T = [
  [X_START, 0.0], [-3.95, 0.01], [-3.2, 0.08], [1.6, 0.09], [4.0, 0.05], [X_END, 0.02],
];
...
export function createAirflow() {
  ...
  const plumeMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main(){
        float t = clamp((vP.x - 4.0) / 5.0, 0.0, 1.0);
