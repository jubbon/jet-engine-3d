...
```json
{
  ...
  "control": {
    ...
    "starterCutoffN2": 0.30, "lightOffN2": 0.22,
    "spoolUpSec": 4.0, "spoolDownSec": 2.6
  },
...
(обороты, число лопаток, диаметр вентилятора), остальные проставлены по
порядку величины и подлежат сверке с источниками при заполнении настоящего
файла. Именно для этого в `meta` есть `sources` и `confidence` со значениями
