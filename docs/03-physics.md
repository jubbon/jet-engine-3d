...
Реальные обороты — 3300 об/мин (N1) и порядка 15 000 об/мин (N2), то есть
345 и 1571 рад/с. Отрисовать их невозможно: при 60 кадрах в секунду вентилятор
за кадр поворачивался бы на 5.75 рад, а угловой шаг между 22 лопатками — 0.29 рад.
...
где `n1`, `n2` — доли от максимальных оборотов. Соотношение скоростей каскадов
сжато (1.7 вместо реальных 4.5) — иначе ротор ВД снова упёрся бы в стробоскоп.
Приборы при этом показывают честные проценты оборотов.
...
```
f = (об/мин × число лопаток) / 60 = (n1 · 3300 · 22) / 60
```

От 267 Гц на малом газе до 1210 Гц на взлётном. Это реальный физический
механизм: каждая лопатка, проходя мимо неподвижной точки, создаёт импульс
давления. Проверено измерением спектра: энергия на расчётной частоте превышает
соседние полосы в 12–60 раз.
