...
подрезает низ сечения плавным минимумом на `BELLY` = 0.2 у.е., профиль капота
...
Осталось четыре вещи, и первая из них — не украшение, а видимый дефект.
...
* **Габариты по источникам.** Наружный радиус (2.0 у.е. = 1.0 м) и глубина
  среза `BELLY` подобраны по пропорции, а не взяты из данных. Наружный диаметр,
  длина гондолы, вынос относительно кромки крыла и просвет до земли стоит
  выписать в таблицу со ссылками — тогда и сплющивание получит настоящую
  величину, а не правдоподобную.
...
**Прототип по-прежнему смешанный.** Документация называет прототипом CFM56-7B,
и число лопаток вентилятора (24) этому отвечает, а число ступеней — 3 подпорные,
10 КВД, 2 ТВД, 5 ТНД — это компоновка LEAP-1B, то есть 737 MAX. Для гондолы
